...
| [`workflows/source-monitor-poller.json`](workflows/source-monitor-poller.json) | Source Monitor — Durable-Cursor Poller | poll → read config → read durable cursors → per-adapter fetch → normalize to a canonical item → dedup against a seen table → advance cursors. The reusable INGEST half of a source-watcher: "watch these firehoses and hand me only the new, deduped items". A config-driven `SourceAdapter` descriptor adds a source as data, not a new subgraph. Emits the canonical `{ source, external_id, url, title, body, author, posted_at, content_hash }` stream; everything past cursor-advance is the CONSUMING brick (qualify/notify/diff). Extracted from the Radar scraper base; reused by the SEO health-monitor (firecrawl-diff) class. |
...
| [`workflows/content-tracker.json`](workflows/content-tracker.json) | Content / On-Page Tracker (`[content_tracker_plugin]`) | webhook → dual HMAC verify → 401 gate → signed 202 ack → spend guard → Firecrawl scrape on-page signals (object-first, honest-empty) → DataForSEO Labs bulk difficulty for target terms → deterministic `CONTENT-<class>-<n>` content issues → fail-open LLM content-gap seam → findings-ledger UPSERT → signed notify. New-site-friendly (no rankings required). |
| [`workflows/competitor-gap.json`](workflows/competitor-gap.json) | Competitor / SERP Gap (`[competitor_gap_plugin]`) | webhook → dual HMAC verify → 401 gate → signed 202 ack → spend guard → optional SERP-overlap competitor auto-derive → DataForSEO Labs competitor/gap pull (object-first, honest-empty) → stable `GAP-<class>-<n>` finding ids → fail-open LLM cluster/prioritize seam → gaps-ledger UPSERT → signed notify. |
| [`workflows/seo-monitor.json`](workflows/seo-monitor.json) | SEO Health Monitor (`[seo_monitor_plugin]`) | schedule → Confirm Config → spend guard → Firecrawl crawl (object-first, honest-empty) → read prior snapshot → diff → classify stable-ID change events (indexability/robots/canonical/status/title/meta/sitemap) → snapshot + events UPSERT → alert only on real movement via signed notify. |
| [`workflows/backlinks.json`](workflows/backlinks.json) | Backlinks (`[backlinks_plugin]`) | webhook + weekly schedule → dual HMAC verify → 401 gate → signed 202 ack → spend guard → DataForSEO Backlinks summary + referring_domains + domain_intersection link gaps (object-first, honest-empty) → stable `LINK-<hex>` finding ids → fail-open LLM prioritize seam → backlink-findings ledger UPSERT → signed notify. |
| [`workflows/content-gen.json`](workflows/content-gen.json) | Content Generation / Briefs (`[content_gen_plugin]`) | webhook → dual HMAC verify → 401 gate → signed 202 ack → spend guard → DataForSEO SERP (object-first, honest-empty) → grounding (top-ranking titles/entities/features/intent) → stable `BRIEF-<hex>` id → fail-open LLM generation seam (honest placeholder when unbound) → DRAFT-FIRST content-briefs ledger UPSERT → signed notify. |
| [`workflows/reporting-digest.json`](workflows/reporting-digest.json) | Reporting Digest (`[reporting_digest_plugin]`) | weekly schedule → read prior report-log → fresh-period dedup guard → read the SEO-audit orchestrator's latest run + prioritized queue (`tmpl_seo_audit_runs` / `tmpl_seo_audit_queue`, honest-empty) → deterministic exec-summary facts → fail-open LLM `executeWorkflow` summarize seam → compose digest → signed notify → stamp + UPSERT the report-log ledger. Pure reader over the orchestrator output; no spend gate, no external metered pull. |
| [`workflows/gsc-csv.json`](workflows/gsc-csv.json) | GSC Integration — CSV stopgap (`[gsc_csv_plugin]`) | webhook → dual HMAC verify → 401 gate → signed 202 ack → parse the uploaded GSC Performance CSV → normalize rows → UPSERT `tmpl_gsc_performance` → deterministic CTR-outlier + query-cannibalization findings → UPSERT `tmpl_seo_gsc_findings` → fail-open LLM draft-suggestions seam (honest placeholder when unbound) → signed notify. No metered API — the client's own GSC export is the fact source (no spend gate). |
