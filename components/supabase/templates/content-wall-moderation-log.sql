-- =============================================================================
-- TEMPLATE content-wall-moderation-log — [TEMPLATE] Content Wall moderation log ([content_wall_plugin] brick, audit, service-role-only)
-- §8.1 shape (1): APP DATA Shopify does not own.
-- RLS by default; REVOKE discipline; idempotent. Client-AGNOSTIC slots only.
-- =============================================================================

-- SELF-CONTAINED PARENT CHAIN: emit tmpl_content_wall (+ its public view) FIRST so the

-- FK resolves when the canary applies THIS migration to an isolated scratch schema.

-- Reused verbatim from the content-wall builder (idempotent); do not hand-edit here.
...
-- STUDIO-OWNED [content_wall_plugin] moderation audit trail (app data Shopify does not own,

-- §8.1 shape 1), EXCLUSIVE to the brick. Server-write-only TABLE-WIDE: NOT even

-- authenticated reads it — the moderation dashboard / Triage callback read+write server-side

-- via service_role. The verdict is ALWAYS a guarded service_role write; anon NEVER approves.

create extension if not exists pgcrypto;

create table if not exists "public"."tmpl_content_wall_moderation_log" (
  "id" uuid primary key default gen_random_uuid(),
  "content_id" uuid not null references "public"."tmpl_content_wall" (id) on delete cascade,
  "reviewer_id" text,
...
alter table "public"."tmpl_content_wall_moderation_log" enable row level security;

revoke all on "public"."tmpl_content_wall_moderation_log" from anon, public;

grant select, insert, update, delete on "public"."tmpl_content_wall_moderation_log" to service_role;

drop policy if exists "tmpl_content_wall_moderation_log_service_all" on "public"."tmpl_content_wall_moderation_log";
create policy "tmpl_content_wall_moderation_log_service_all"
  on "public"."tmpl_content_wall_moderation_log" for all
  to service_role
  using (true)
  with check (true);
