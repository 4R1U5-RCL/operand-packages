-- =============================================================================
-- TEMPLATE reactivation-replies — [TEMPLATE] Reactivation replies ([reactivation_plugin] brick, classified inbound, service-role-only)
...
-- STUDIO-OWNED [reactivation_plugin] classified inbound replies (app data Shopify does not

-- own, §8.1 shape 1), EXCLUSIVE to the brick. NO reply body — intent + telemetry only.

-- UNIQUE (provider_msg_id) makes webhook redelivery a no-op. Server-write-only TABLE-WIDE.

create extension if not exists pgcrypto;

create table if not exists "public"."tmpl_reactivation_replies" (
  "id" uuid primary key default gen_random_uuid(),
  "contact_id" uuid not null references "public"."tmpl_reactivation_contacts" (id) on delete cascade,
  "channel" text not null check (channel in ('email','sms')),
  "intent" text check (intent is null or intent in ('booking_request','interested','unsubscribe','other')),
  "confidence" real,
...
create unique index if not exists "tmpl_reactivation_replies_provider_msg_uidx" on "public"."tmpl_reactivation_replies" ("provider_msg_id");

create index if not exists "tmpl_reactivation_replies_contact_channel_idx" on "public"."tmpl_reactivation_replies" ("contact_id", "channel");

alter table "public"."tmpl_reactivation_replies" enable row level security;

revoke all on "public"."tmpl_reactivation_replies" from anon, public;

grant select, insert, update, delete on "public"."tmpl_reactivation_replies" to service_role;

drop policy if exists "tmpl_reactivation_replies_service_all" on "public"."tmpl_reactivation_replies";
create policy "tmpl_reactivation_replies_service_all"
  on "public"."tmpl_reactivation_replies" for all
  to service_role
  using (true)
  with check (true);
