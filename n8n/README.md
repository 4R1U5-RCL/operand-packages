# n8n — reusable workflow templates

A library of **reusable n8n workflow templates** for the studio's hosted n8n
instance. Each template is a node-graph pattern distilled from the workflows we
actually run (`[STUDIO_TESSERA]`, `[STUDIO_NOTIFICATIONS]`, `[TESSERA]`,
`[MOSAIC]`, `[SCARLET]`) — built once to serve as the starting point and the
reference shape for future workflow builds.

These are **templates, not live workflows**: they ship **inactive** and with
**unbound credential slots**. Binding credentials and activating is a deliberate
human/ops step when a template is instantiated for a specific job.

> Source of truth: the JSONs here are exported from the hosted
> [`PACKAGE/Templates`](https://csco.app.n8n.cloud) n8n project
> (`IKEgTeej0upY4GVP`). They are authored as code in the studio monorepo
...
> hand-drawn. Treat this dir as a published, importable snapshot.
...
- **No secrets, no hardcoded URLs.** Credentials are unbound slots
  (`nodeCredentialType` only); instance URLs and tokens are `$env`/parameter
  placeholders. Bind them at instantiation.
- **Boundaries.** Templates trigger/display and read live; they do **not** embed
  scraping/structuring pipelines, and they never write Shopify's commercial state
  into Supabase as a source of truth.
...
4. Rename + activate when you're ready. Keep the `[TEMPLATE]` originals inactive.

To regenerate this snapshot from the hosted project, re-export the
`PACKAGE/Templates` workflows (or re-run the harness `n8n-template` build, which
provisions them idempotently).
